...
def benchmark_roberta(batch_size=1, iterations_per_epoch=10):
    print("Running benchmark for RoBERTa...")
    iteration_times = []
    ...
    for iteration in range(iterations_per_epoch):
        ...
        np.savez("temp_model_weights.npz", *model_weights_cpu)  # 将模型权重保存到文件
        ...
        print(f"Iteration {iteration + 1} - Time taken for snapshot: {snapshot_time:.2f} seconds")
        print(f"Iteration {iteration + 1} - Time taken for persist: {persist_time:.2f} seconds")
    ...
    print(f"Total training time for 1 epoch with RoBERTa (excluding first iteration):", sum(iteration_times))

    return sum(iteration_times)

# 运行基准测试并保存结果
total_time = benchmark_roberta()
with open("RoBERTa_performance.txt", "w") as f:
    f.write(f"Total Time (excluding first iteration): {total_time:.2f} seconds\n")

print("Benchmark results saved to RoBERTa_performance.txt")

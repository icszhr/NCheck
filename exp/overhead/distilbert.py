...
def benchmark_distilbert(batch_size=1, iterations_per_epoch=10):
    print("Running benchmark for DistilBERT...")
    iteration_times = []
    ...
    print(f"Total training time for 1 epoch with DistilBERT (excluding first iteration):", sum(iteration_times))

    return sum(iteration_times)

# 运行基准测试并保存结果
total_time = benchmark_distilbert()
with open("DistilBERT_performance.txt", "w") as f:
    f.write(f"Total Time (excluding first iteration): {total_time:.2f} seconds\n")

print("Benchmark results saved to DistilBERT_performance.txt")

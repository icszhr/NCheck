...
for idx, model_config in enumerate(models_config[:4]):
    model_name, _, model_class, model_params = model_config
    batch_size = 2 ** idx
    total_time, total_checkpoint_time = benchmark_model(model_name, None, model_class, model_params, batch_size=batch_size, checkpoint_interval=10)
    results.append({
        "original_model_name": model_name,
        "simulated_model_name": "ResNet50",
...
for idx, model_config in enumerate(models_config[4:]):
    ...
    batch_size = 2 ** idx
    total_time, total_checkpoint_time = benchmark_model(model_name, tokenizer_class, model_class, model_params, batch_size=batch_size, checkpoint_interval=10)
    results.append({
        "original_model_name": model_name,
        "simulated_model_name": "BERT",
        "batch_size": batch_size,
        "total_time": total_time,
        "total_checkpoint_time": total_checkpoint_time
    })
...
# 输出结果到JSON文件
with open("result1_2.json", "w") as f:
    json.dump(results, f, indent=4)

print("Benchmark results saved to result1_2.json")

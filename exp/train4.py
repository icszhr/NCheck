...
class CheckpointAndTimingCallback(tf.keras.callbacks.Callback):
    def __init__(self, model, checkpoint_dir, save_checkpoints=True):
        ...
        self.cpu_weights = None  # 添加这一行来存储权重
    ...
    def save_checkpoint(self, iteration):
        ...
        def snapshot_weights():
            ...
            self.cpu_weights = {var.name: var.value().numpy() for var in self.model.variables}
        ...
        def persist_weights():
            ...
            np.savez(checkpoint_name, **self.cpu_weights)
        ...
        self.persist_thread = threading.Thread(target=persist_weights)
...
def calculate_tensor_size(tensor):
    return np.prod(tensor.shape) * np.dtype(tensor.dtype.as_numpy_dtype).itemsize
...
def benchmark_model(model_name, tokenizer_class, model_class, pretrained_model_name, batch_size=8, iterations_per_epoch=10):
    ...
    if model_name == "ResNet50":
        ...
        for iteration in range(iterations_per_epoch):
            ...
            for images, labels in train_dataset.take(1):  # 只进行一个 iteration
                ...
                # 计算GPU内存使用峰值
                current_memory_usage = sum(calculate_tensor_size(var) for var in model.trainable_variables) / (1024 ** 2)
    ...
    return sum(iteration_times), checkpoint_blocking_time, peak_memory_usage
...
for model_config in models_config:
    ...
    total_time, total_checkpoint_time, peak_memory_usage = benchmark_model(model_name, tokenizer_class, model_class, pretrained_model_name)
    results.append({
...
with open("result3.txt", "w") as f:
    for result in results:
        ...
        f.write(f"Total Checkpoint Blocking Time: {result['total_checkpoint_time']:.2f} seconds\n")
        f.write(f"Peak GPU Memory Usage: {result['peak_memory_usage']:.2f} MB\n")
        f.write("\n")

print("Benchmark results with blocking and GPU memory usage saved to result3.txt")

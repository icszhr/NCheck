...
/*void AlignedFree(void* aligned_memory) { 
  std::lock_guard<std::mutex> lock(mutex_);
        auto it = allocations.find(aligned_memory);
        if (it != allocations.end()) {
            //LOG(INFO)<< "Deallocated: " << aligned_memory << std::endl;
            Free(aligned_memory);
            allocations.erase(it);
        }
        else {
        LOG(INFO)<< "Attempted to deallocate unknown pointer: " << aligned_memory << std::endl;
    }
 }*/
...
std::size_t MallocExtension_GetAllocatedSize(const void* p) { 
  std::lock_guard<std::mutex> lock(mutex_);
        auto it = allocations.find(const_cast<void*>(p));
        if (it != allocations.end()) {
            return it->second;
        }
        return 0;
